...
public class RailwayLinkAngleIsSuitablePredicate implements BiPredicate<RailwayLink, RailwayLink> {
	...
	private DirectedGraph<RailwayBaseNode<?>, RailwayLink> railwayGraph;

	// 180 * Vector2D.angle(lastVector, firstVector) / Math.PI

	public RailwayLinkAngleIsSuitablePredicate(DirectedGraph<RailwayBaseNode<?>, RailwayLink> railwayGraph) {
	...
	public boolean test(RailwayLink firstLink, RailwayLink secondLink) {
		...
		RailwayBaseNode<?> firstLinkTarget = railwayGraph.getEdgeTarget(firstLink);
		RailwayBaseNode<?> firstLinkSource = railwayGraph.getEdgeSource(firstLink);
		RailwayBaseNode<?> secondLinkTarget = railwayGraph.getEdgeTarget(secondLink);
		RailwayBaseNode<?> secondLinkSource = railwayGraph.getEdgeSource(secondLink);

		if (Objects.equals(firstLinkTarget, secondLinkSource) && Objects.equals(firstLinkSource, secondLinkTarget) && firstLinkTarget.canTurn()) {
			return true;
		}
		...
		if (angle < MAX_ALLOWED_ANGLE) {
			if (angle > WARNING_TRESHHOLD_ANGLE) {
				System.out.println(MessageFormat.format(
						...
						firstLink.getProperties().getId(), lon0, lat0, secondLink.getProperties().getId(), lon1, lat1,
						(angle / Math.PI) * 180));
			}
			return true;
		} else {
